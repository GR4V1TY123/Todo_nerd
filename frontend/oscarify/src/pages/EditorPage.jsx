    ...
    // STEP 2: If button says "Check" - submit delivery proof with stored intent ID
      ...
      try {
        const proofResponse = await fetch(
          `http://api.fmm.finternetlab.io/api/v1/payment-intents/${currentIntentId}/escrow/delivery-proof`,
          {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
              ...
              proofURI: `http://example.com/delivery-proofs/${currentIntentId}`,
        ...
        <div className="editor-canvas">
          <div className={`editor-content ${isTransitioning ? 'transitioning' : ''
            ...
            <div className="editor-quick-actions">
              ...
              <button
                className={`quick-action-btn editor-tts-btn${isSpeaking ? ' speaking' : ''}`}
                onClick={() => {
                  const text = editorRef.current ? editorRef.current.innerText : ''
                  if (text.trim()) handleTTS(text)
                }}
                title={isSpeaking ? 'Stop speaking' : 'Read chapter aloud'}
                style={{ marginLeft: 'auto' }}
              >
                {isSpeaking ? '⏹' : '🔊'}
                <span style={{ marginLeft: 4 }}>{isSpeaking ? 'Stop' : 'Listen'}</span>
              </button>
            </div>
          </div>
        </div>
